...
- Examples using Quantum ESPRESSO requires the installation of the `aiida-quantumespresso` plugin, and at least of a pw.x executable.
- Examples with the `aiida-defect` workflows requires the installation of the aiida-defect plugin by Conrad Johnston. This plugin requires the aiida-quantum espresso, and installed ph.x and pp.x from Quantum ESPRESSO.
- Of course, in general when you add a workflow, you need to have installed the plugin and a code! The additional hurdle for this is to add a dictionary of property-workflow mapping.
...
If you inspect the ``search_HfO2_strict.json`` you see that the file contains some wrong keywords, as ``schemical``, ``somedata``, that are filtered out by the workflow.
